...
개발 과정에서 무심코 작성한 SQL이 운영 환경에서 심각한 성능 저하를 일으키는 경우가 많습니다. 본 문서에서는 실무에서 자주 발견되는 **20가지 대표적인 SQL 안티패턴(실수)**과 이를 올바르게 개선하는 튜닝 팁을 난이도별(초/중/고급)로 분류하여 소개합니다.
...
**To-Be (IS NOT NULL 명시 또는 NOT EXISTS 활용)**
```sql
...
**To-Be (올바른 인라인 뷰 사용 및 부분범위 처리 유도)**
```sql
...
**To-Be**
- **파티셔닝(Partitioning):** 월별/일별로 테이블을 분할(Range Partition)하여 오래된 파티션만 쉽게 Drop
- **아카이빙(Archiving):** 3개월이 지난 데이터는 백업용 아카이브 테이블로 옮기고 메인 테이블에서 삭제 (주기적 Job 구성)
